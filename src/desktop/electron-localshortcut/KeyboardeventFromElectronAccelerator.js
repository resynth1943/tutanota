...
export type Event = {key?: string, code?: string, metaKey?: boolean, ctrlKey?: boolean, altKey?: boolean, shiftKey?: boolean}
export type ReducedEvent = {event: Event, accelerator: string}
...
export function reduceCode({accelerator, event}: ReducedEvent, {code, key}: {code: string, key: string}): ReducedEvent {
	if (event.code) {
...
export function toKeyEvent(accelerator: string): {} {
	let state: ReducedEvent = {accelerator, event: {}};
